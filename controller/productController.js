...
const getAllProducts = async (req, res) => {
  ...
  // queryObject.store = req.user._id;
...
const getAllProductsForSuperAdmin = async (req, res) => {
  const { title, category, price, page, limit } = req.query;
  //  console.log('title',title)
  let queryObject = {};
  let sortObject = {};
...
const getAllProductApprovalRequests = async (req, res) => {
  const { title, category, price, page, limit } = req.query;

   console.log('title in get all product approvals')
  let queryObject = {};
  let sortObject = {};
  if (title) {
    queryObject.$or = [
...
const updateStatus = (req, res) => {
  ...
  if(newStatus=="show"){
    console.log("New approval added!")
  }
  Product.updateOne(
    { _id: req.params.id },
    {
...
const updateApprovalStatus = (req, res) => {
  // const newStatus = 
  const newStatus = req.body.approved;
  let updateData = {}
  if(newStatus == false){
    updateData = {
      status: 'hide',
  ...
  Product.updateOne(
    ...
    (err) => {
      if (err) {
        ...
        res.status(200).send({
          message: `Product Approved/Rejected Successfully!`,
        });
      }
    }
  );
