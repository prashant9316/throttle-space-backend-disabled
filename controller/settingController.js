...
const getGlobalSetting = async (req, res) => {
  try {
    ...
    console.log(storeId)
    const globalSetting = await Setting.findOne({ name: "globalSetting", store: storeId });
    ...
    const globalSetting = await Setting.updateOne(
      ...
      },
      {
        $set: {
          ...
          "setting.shop_owner_name": req.body.setting.shop_owner_name,
          "setting.company_description": req.body.setting.company_description,
          "setting.gst_number": req.body.setting.gst_number,
          "store": req.user._id
        },
      }
    );
